...
type Pedido = {
  ...
  canal?: string | null;   // SITE | PDV
  comanda?: string | null; // número comanda
...
export default function CupomPedidoDF() {
  ...
  const cupomUrl = useMemo(() => {
    if (!id) return "";
    return `https://${SITE}/dfdistribuidora/cupom/${id}`;
  }, [id]);
  ...
  const subtotalCalc = useMemo(() => {
    const sum = itens.reduce((s, i) => {
      const q = safeNumber((i as any).qtd ?? (i as any).quantidade ?? 0);
      const sub =
        i.subtotal !== null && i.subtotal !== undefined ? safeNumber(i.subtotal) : safeNumber(i.preco) * q;
      return s + sub;
    }, 0);
    return sum;
  }, [itens]);
  ...
  async function carregarPedidoEItens(pedidoId: string) {
    ...
    const { data: p, error: e1 } = await supabase.from("df_pedidos").select("*").eq("id", pedidoId).single();
    ...
    if (AUTO_PRINT) setTimeout(() => window.print(), 350);
  }
    ...
    <div className="min-h-screen bg-gray-100 p-3">
      <style>{`
        @media print {
          ...
          .paper { box-shadow: none !important; border: none !important; margin: 0 !important; color: #000 !important; }
        ...
        <div className="max-w-md mx-auto paper bg-white border rounded-xl p-4 mono text-black">
          ...
          <div className="mt-3 grid grid-cols-2 gap-3 items-start">
            ...
            <div className="border rounded-lg p-2">
              ...
              <div className="mt-1 text-[11px] font-extrabold text-black text-center">{idCurto || "-"}</div>
            </div>
          </div>
          ...
          <div className="text-sm text-black">
            ...
            {itens.length === 0 ? (
              <div className="text-xs text-black">Sem itens (campo itens vazio).</div>
            ) : (
              <div className="space-y-1">
                {itens.map((i, idx) => {
                  const q = safeNumber((i as any).qtd ?? (i as any).quantidade ?? 0);
                  const preco = safeNumber(i.preco);
                  const sub = i.subtotal !== null && i.subtotal !== undefined ? safeNumber(i.subtotal) : preco * q;
