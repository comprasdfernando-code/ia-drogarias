...
type Pedido = {
  ...
  canal?: string | null;     // PDV | SITE
  comanda?: string | null;   // número da comanda

  cliente_nome?: string | null;
  cliente_whatsapp?: string | null;

  tipo_entrega?: "ENTREGA" | "RETIRADA" | string;
...
const STATUS_STYLE: Record<string, { badge: string; card: string; btn: string; label: string }> = {
  ...
  FECHADO_PDV: {
    badge: "bg-gray-200 text-gray-950 border border-gray-300",
    card: "border-l-8 border-gray-700",
...
export default function PainelPedidosDF() {
  ...
  async function mudarStatus(id: string, status: string) {
    const { error } = await supabase.from("df_pedidos").update({ status }).eq("id", id);
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel("df-painel-pedidos")
      .on("postgres_changes", { event: "*", schema: "public", table: "df_pedidos" }, async (payload) => {
        await carregar();
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4">
        <div>
          <h1 className="text-2xl font-extrabold text-gray-950">📥 Painel de Pedidos – DF</h1>
          <p className="text-sm text-gray-800">SITE + PDV • tempo real • cupom e comanda</p>
        </div>
        ...
        <div className="flex gap-2">
          ...
          >
            ...
            <option value="FECHADO_PDV">Fechado PDV</option>
      ...
      <div className="space-y-3">
        {filtrados.map((p) => {
          ...
          const canalLabel = (p.canal || "SITE").toUpperCase();
          ...
          return (
            <div key={p.id} className={`bg-white rounded-2xl border border-gray-200 shadow-sm p-4 ${st.card}`}>
              <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className="text-xs px-2 py-1 rounded-full font-extrabold bg-gray-100 border text-gray-950">
                      Canal: {canalLabel}
                    </span>

                    {p.comanda ? (
                      <span className="text-xs px-2 py-1 rounded-full font-extrabold bg-black text-white">
                        Comanda: {p.comanda}
                      </span>
                    ) : null}
                  </div>
                  ...
                  <p className="text-sm mt-1 text-gray-950 font-semibold">
                    <span className="font-extrabold">{brl(Number(p.total || 0))}</span>
                  ...
                  <div className="mt-3 text-sm bg-gray-50 border border-gray-200 rounded-xl p-3">
                    <div className="flex items-center justify-between gap-2 flex-wrap">
                      ...
                      <div className="text-xs text-gray-800 font-semibold">
                        Subtotal: {brl(Number(p.subtotal || 0))} • Taxa: {brl(Number(p.taxa_entrega || 0))} • Total:{" "}
                        <span className="font-extrabold text-gray-950">{brl(Number(p.total || 0))}</span>
                      </div>
                    </div>
                          ...
                          itens.map((i: any, idx: number) => (
                            <div key={idx} className="flex justify-between gap-3">
                              <span className="text-gray-950 font-semibold truncate">
                                {Number(i.qtd || 0)}x {String(i.nome || "")}
                              </span>
                ...
                <div className="flex flex-row lg:flex-col items-stretch lg:items-end gap-2">
                  <Link
                    href={`/dfdistribuidora/cupom/${p.id}`}
                    target="_blank"
              ...
              <div className="flex gap-2 mt-4 flex-wrap">
                {(["FECHADO_PDV", "NOVO", "PREPARO", "PRONTO", "ENTREGUE"] as const).map((s) => (
                  <button
                    key={s}
