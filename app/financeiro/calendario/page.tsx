    ...
    <div className="space-y-6">
      <div className="bg-[#112240] border border-cyan-500/20 shadow-md rounded-2xl p-6">
        ...
        <p className="text-slate-400 text-sm mt-1">
          Tudo que entra e sai por dia (visão do Fer)
        </p>
      </div>
        ...
        <table className="w-full text-left text-white">
          ...
          <tbody>
            {Array.from({ length: diasNoMes }, (_, idx) => idx + 1).map((dia) => {
              const items = (porDia.get(dia) || []).sort((a, b) =>
                a.tipo > b.tipo ? 1 : -1
              );
              ...
              return (
                <tr key={dia} className="border-b border-cyan-500/10 align-top">
                  ...
                  <td className="py-3 text-emerald-300 font-semibold">
                    {t.entradas ? brl(t.entradas) : "—"}
                  </td>

                  <td className="py-3 text-amber-300 font-semibold">
                    {t.saidas ? brl(t.saidas) : "—"}
                  </td>

                  <td
                    className={`py-3 font-semibold ${
                      t.saldo >= 0 ? "text-emerald-300" : "text-red-300"
                    }`}
                  >
