...
type FechamentoResumo = {
  ...
  entradasMov: number;
  saidasMov: number;
  divergenciaCartao: number;
  divergenciaPix: number;
  ...
  cieloTaxa: number;
  cieloVendas: number;
  diffCartaoXCieloBruto: number;
  diffCartaoXCieloLiquido: number;
};
...
function importarCieloCSV(texto: string, nomeArquivo: string) {
  ...
  const headerIndex = linhas.findIndex(
    (l) => l.toLowerCase().includes("data da venda") && l.toLowerCase().includes("valor bruto")
  );

  if (headerIndex === -1) return [] as CieloResumoDia[];

  const header = parseCSVLine(linhas[headerIndex], sep).map(normalizarCabecalho);

  const idxData = header.findIndex((h) => h.includes("data"));
  const idxBruto = header.findIndex((h) => h.includes("bruto"));
  const idxLiquido = header.findIndex((h) => h.includes("liquido"));
  const idxTaxa = header.findIndex((h) => h.includes("taxa") || h.includes("tarifa"));
  ...
  for (let i = headerIndex + 1; i < linhas.length; i++) {
    ...
    const taxa = idxTaxa >= 0 ? Math.abs(parseBRNumber(cols[idxTaxa])) : 0;
...
export default function CaixaFechamentoPage() {
  ...
  const resumidos = useMemo<FechamentoResumo[]>(() => {
    return fechamentos
      .map((f) => {
        ...
        const sessaoEncontrada = sessaoId ? sessoesMap.get(sessaoId) || null : null;
  ...
  }, [fechamentos, movimentacoes, sessoesMap, cieloMap]);
  ...
  const sessoesDisponiveis = useMemo(() => {
    ...
    return unicos.map((id) => {
      ...
      return {
        id,
        label: `${String(id).slice(0, 8)}${s?.operador ? ` • ${s.operador}` : ""}${s?.turno ? ` • ${s.turno}` : ""}`,
      };
    });
  ...
  const resumoSaidasDestino = useMemo(() => {
    const entradas = movimentacoesFiltradas.filter((m) => m.tipo === "Entrada");
    const saidas = movimentacoesFiltradas.filter((m) => m.tipo === "Saída");

    const entradaDinheiro = entradas
      .filter((m) => (m.destino_financeiro || "") === "CAIXA_DINHEIRO" || (m.forma_pagamento || "") === "Dinheiro")
      .reduce((t, m) => t + Number(m.valor || 0), 0);

    const entradaBradesco = entradas
      .filter((m) => (m.destino_financeiro || "") === "CONTA_BRADESCO")
      .reduce((t, m) => t + Number(m.valor || 0), 0);

    const entradaCartao = entradas
      .filter((m) => {
        const forma = (m.forma_pagamento || "").toLowerCase();
        return forma.includes("cart") || forma.includes("cartão");
      })
    ...
    const entradaPix = entradas
      .filter((m) => (m.forma_pagamento || "").toLowerCase().includes("pix"))
      .reduce((t, m) => t + Number(m.valor || 0), 0);

    const entradaFiado = entradas
      .filter((m) => (m.forma_pagamento || "").toLowerCase().includes("fiado"))
      .reduce((t, m) => t + Number(m.valor || 0), 0);

    const saidaDinheiro = saidas
      .filter((m) => (m.destino_financeiro || "") === "CAIXA_DINHEIRO")
      .reduce((t, m) => t + Number(m.valor || 0), 0);

    const saidaBradesco = saidas
      .filter((m) => (m.destino_financeiro || "") === "CONTA_BRADESCO")
      .reduce((t, m) => t + Number(m.valor || 0), 0);

    const porTipo = {
      sangriasDinheiro: saidas
        .filter((m) => (m.destino_financeiro || "") === "CAIXA_DINHEIRO" && (m.descricao || "").toLowerCase().includes("sangria"))
        .reduce((t, m) => t + Number(m.valor || 0), 0),
      despesasDinheiro: saidas
        .filter((m) => (m.destino_financeiro || "") === "CAIXA_DINHEIRO" && (m.descricao || "").toLowerCase().includes("desp"))
        .reduce((t, m) => t + Number(m.valor || 0), 0),
      boletosDinheiro: saidas
        .filter((m) => (m.destino_financeiro || "") === "CAIXA_DINHEIRO" && (m.descricao || "").toLowerCase().includes("boleto"))
        .reduce((t, m) => t + Number(m.valor || 0), 0),
      comprasDinheiro: saidas
        .filter((m) => (m.destino_financeiro || "") === "CAIXA_DINHEIRO" && (m.descricao || "").toLowerCase().includes("compra"))
        .reduce((t, m) => t + Number(m.valor || 0), 0),
      sangriasBradesco: saidas
        .filter((m) => (m.destino_financeiro || "") === "CONTA_BRADESCO" && (m.descricao || "").toLowerCase().includes("sangria"))
        .reduce((t, m) => t + Number(m.valor || 0), 0),
      despesasBradesco: saidas
        .filter((m) => (m.destino_financeiro || "") === "CONTA_BRADESCO" && (m.descricao || "").toLowerCase().includes("desp"))
        .reduce((t, m) => t + Number(m.valor || 0), 0),
      boletosBradesco: saidas
        .filter((m) => (m.destino_financeiro || "") === "CONTA_BRADESCO" && (m.descricao || "").toLowerCase().includes("boleto"))
        .reduce((t, m) => t + Number(m.valor || 0), 0),
      comprasBradesco: saidas
        .filter((m) => (m.destino_financeiro || "") === "CONTA_BRADESCO" && (m.descricao || "").toLowerCase().includes("compra"))
        .reduce((t, m) => t + Number(m.valor || 0), 0),
    };

    return {
      totalEntradas: entradas.reduce((t, m) => t + Number(m.valor || 0), 0),
      totalSaidas: saidas.reduce((t, m) => t + Number(m.valor || 0), 0),
      entradaDinheiro,
      entradaBradesco,
      entradaCartao,
      entradaPix,
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-6">
          <div>
            ...
            <p className="text-sm text-gray-600">
              Painel completo por fechamento, com sessão, operador, turno e comparação com extrato da Cielo.
            </p>
          </div>
        ...
        <div className="bg-white rounded-xl shadow p-4 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-3">
            ...
            <input
              ...
              placeholder="Buscar descrição, operador, valor..."
        ...
        <div className="bg-white rounded-xl shadow p-4 mb-6">
          <div className="flex flex-col lg:flex-row lg:items-end gap-4 justify-between">
            <div>
              ...
              <p className="text-sm text-gray-600">
                Importe o CSV baixado da Cielo para comparar cartão do sistema com bruto e líquido por dia.
              </p>
            </div>
            <div className="flex flex-wrap gap-3">
              <label className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-semibold cursor-pointer">
                ...
                <input
                  ...
                  accept=".csv,text/csv"
                  className="hidden"
                  onChange={(e) => onArquivoCielo(e.target.files?.[0] || null)}
                />
              </label>
              <button
                onClick={exportarFechamentosCSV}
        ...
        <div className="bg-white rounded-xl shadow p-4 mb-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-bold text-lg text-blue-700">🏦 Relatório de Saídas por Origem do Dinheiro</h2>
            <span className="text-xs text-gray-500">Todos os caixas somados no filtro atual</span>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-4">
            <CardResumo titulo="Entradas Totais" valor={resumoSaidasDestino.totalEntradas} cor="green" />
            <CardResumo titulo="Saídas Totais" valor={resumoSaidasDestino.totalSaidas} cor="red" />
            <CardResumo titulo="Saldo Caixa Dinheiro" valor={resumoSaidasDestino.saldoDinheiro} cor={resumoSaidasDestino.saldoDinheiro >= 0 ? "green" : "red"} />
            <CardResumo titulo="Saldo Conta Bradesco" valor={resumoSaidasDestino.saldoBradesco} cor={resumoSaidasDestino.saldoBradesco >= 0 ? "green" : "red"} />
            <CardResumo titulo="Saldo Consolidado" valor={resumoSaidasDestino.saldoConsolidado} cor={resumoSaidasDestino.saldoConsolidado >= 0 ? "blue" : "red"} />
            <CardResumo titulo="Entradas em Dinheiro" valor={resumoSaidasDestino.entradaDinheiro} cor="green" />
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-4">
            <CardResumo titulo="Entradas Pix" valor={resumoSaidasDestino.entradaPix} cor="green" />
            <CardResumo titulo="Entradas Cartão" valor={resumoSaidasDestino.entradaCartao} cor="green" />
            <CardResumo titulo="Entradas Bradesco" valor={resumoSaidasDestino.entradaBradesco} cor="green" />
            <CardResumo titulo="Entradas Fiado" valor={resumoSaidasDestino.entradaFiado} cor="orange" />
            <CardResumo titulo="Saídas Bradesco" valor={resumoSaidasDestino.saidaBradesco} cor="red" />
          </div>
                ...
                {diasFiltrados.map((item) => {
                  ...
                  return [
                    <tr key={item.key} className="hover:bg-gray-50">
                      ...
                      <td className={`p-2 border text-right font-semibold ${item.diffCartaoXCieloLiquido > 0.009 ? "text-red-700" : "text-green-700"}`}>
                        R$ {fmt(item.diffCartaoXCieloLiquido)}
                      </td>
                      ...
                      <td className="p-2 border text-center">
                        <div className="flex gap-2 justify-center">
                          ...
                          <button
                            onClick={() => setExpandido((prev) => ({ ...prev, [item.key]: !prev[item.key] }))}
                            className="bg-slate-700 hover:bg-slate-800 text-white px-3 py-1 rounded text-xs font-semibold"
                          >
                  ...
                  {movimentacoesDia.length === 0 && (
                    <tr>
                      <td className="p-4 text-center text-gray-500" colSpan={8}>
                        Nenhuma movimentação encontrada nesse fechamento.
                      </td>
                    </tr>
                  )}
                </tbody>
