...
export default function Page() {
  ...
  const [resultado, setResultado] = useState<Produto[]>([]); // produtos filtrados
  ...
  useEffect(() => {
    fetch("/data/produtos.json")
      ...
      .then((data) => {
        setProdutos(data);
        setResultado(data); // inicia mostrando todos
      });
  }, []);

  // Filtrar produtos conforme digita
  useEffect(() => {
      ...
      <header className="sticky top-0 z-50 w-full shadow bg-white">
        ...
        <div className="w-full flex justify-center px-4 py-2 border-t border-gray-200 bg-white">
          <input
            ...
            className="w-[90%] md:w-1/2 px-4 py-2 rounded-l-2xl border border-gray-300 text-sm focus:outline-none"
          />
