      ...
      {!loading && !erro && pedido && (
        <div className="max-w-md mx-auto paper bg-white border rounded-xl p-4 mono">
          ...
          <div className="text-center">
            ...
            <div className="text-xs font-semibold text-black/70">Cupom de Pedido</div>
          </div>
          ...
          <div className="mt-3 grid grid-cols-2 gap-3 items-start">
            <div className="border rounded-lg p-2">
              ...
              <div className="text-[10px] text-black/70 leading-tight">Abrir cupom online</div>
              <div className="mt-2 flex justify-center">
                {qrDataUrl ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={qrDataUrl} alt="QR Code do pedido" className="w-[160px] h-[160px] object-contain" />
                ) : (
                  <div className="text-[11px] text-black/60">Gerando QR...</div>
                )}
              </div>
              <div className="mt-2 text-[10px] text-black/70 break-all">{cupomUrl}</div>
            </div>

            <div className="border rounded-lg p-2">
              <div className="text-[11px] font-bold text-black">Código de barras</div>
              <div className="text-[10px] text-black/70 leading-tight">Conferência rápida</div>
          ...
          <div className="text-sm text-black">
            ...
            <div className="font-semibold">{pedido.cliente_whatsapp || "-"}</div>
            ...
            {String(pedido.tipo_entrega || "").toUpperCase() === "ENTREGA" && (
              <div className="mt-2 text-xs text-black/80">
                ...
                {pedido.referencia && <div className="font-semibold">Ref: {pedido.referencia}</div>}
              </div>
            )}
          </div>
                  ...
                  return (
                    <div key={i.id || `${idx}`} className="text-xs text-black">
                      <div className="flex justify-between gap-2">
                        ...
                        <span className="font-extrabold">{brl(sub)}</span>
                      </div>
