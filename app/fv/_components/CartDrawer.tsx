...
export default function CartDrawer({
  ...
  const total = useMemo(() => (countItems > 0 ? subtotal + taxaEntrega : 0), [subtotal, taxaEntrega, countItems]);

  const msg = useMemo(() => {
    if (items.length === 0) return "Olá! Quero tirar uma dúvida.";
    const lines = items.map(
      (i) => `• ${i.nome} (${i.ean}) — ${i.qtd}x — ${brl(i.preco)}`
    );
...
Pode confirmar a disponibilidade e o prazo?`;
  ...
  return (
    <>
      ...
      {/* drawer */}
      <aside className="fixed right-0 top-0 h-full w-full sm:w-[420px] bg-white z-[90] shadow-2xl flex flex-col">
        <div className="p-4 border-b flex items-center justify-between">
          <div>
            <div className="font-extrabold text-gray-900">Carrinho</div>
              ...
              {items.map((i) => (
                <div key={i.ean} className="border rounded-2xl p-3 flex gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="font-bold text-sm text-blue-950 line-clamp-2">
                      {i.nome}
                    </div>
                    {i.apresentacao && (
                    ...
                    <div className="mt-2 flex items-center gap-2">
                      <button
                        onClick={() => dec(i.ean)}
                        className="w-9 h-9 rounded-lg border hover:bg-gray-50 font-bold"
                      >
                        –
                      </button>
                      <div className="w-10 text-center font-bold">{i.qtd}</div>
                      <button
                        onClick={() => inc(i.ean)}
                        className="w-9 h-9 rounded-lg border hover:bg-gray-50 font-bold"
                      >
        ...
        <div className="border-t p-4">
          <div className="space-y-2 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-semibold">{brl(items.length ? taxaEntrega : 0)}</span>
            </div>
